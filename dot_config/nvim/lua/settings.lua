...
if vim.fn.isdirectory(vim.fn.expand('$HOME/.config/nvim/undo')) == 0 then vim.cmd(
        [[silent !mkdir -p ~/.config/nvim/undo > /dev/null 2>&1]]) end
...
if vim.fn.executable('nvr') then vim.env.VISUAL = "nvr -cc split --remote-wait +'set bufhidden=wipe'" end
